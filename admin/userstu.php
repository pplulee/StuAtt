                ...
                function SearchByName($search){
                    ...
                    if (mysqli_num_rows($result)>0){
                        while ($row = mysqli_fetch_assoc($result)) {
                            echo "<tr><th>" . $row['userid']."</th><td>".$row['username']."</td><td>".$row['email']."</td><td>".$row['class']."</td><td><a href=\"./edituser.php?action=edit&id=" . $row['userid'] . '" class="btn btn-xs btn-info">编辑</a></td>';
                        }
                    }
                }
